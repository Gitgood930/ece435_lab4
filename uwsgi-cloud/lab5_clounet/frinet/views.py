...
def get_posts(request):
    ...
    for u in all_users:
        u_str = str(u)
        u_posts = mc.get(u_str)
        print("request content of cache of {0}: {1}".format(u_str, u_posts))
        if not u_posts:
            print("{0} content has not been cached".format(u_str))
            u_posts = [p for p in Post.objects.filter(user=u)]
            mc.set(u_str, u_posts)
        all_posts += u_posts
...
def make_post(request):
    ...
    # check if cached, and append if it is
    u_str = str(user)
    cached_posts = mc.get(u_str)
    if cached_posts:
        print("adding {0} to {1}'s cache".format(post,u_str))
        cached_posts += [post]
        mc.set(u_str,cached_posts)
    else:
        print("{0} isn't cached, not adding post: {1}".format(u_str,post))
